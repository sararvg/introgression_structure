...
N1 = 100      # number of individuals from pop1
N2 = 100      # number of individuals from pop2
fst = 0.05    # Fst between the two initial populations
allele_number = 2  # number of alleles
num_individuals = range(1, N1 + N2 + 1)  # list of number of indviduals to be analysed
...
num_pop = 0 # set a parameter not to overwrite results and real q values

for dat in glob.glob(os.path.join(mpath + path_pop, '*.dat')):  # access folder with input populations
    with open(dat, 'r') as population:      # open each file in the carpet
        num_pop += 1  # set a number to the population to be analysed
        w = 0 # set a parameter to indicate the number of each line
        i = 0 # set a parameter to indicate the number of each individual
        pop = []  # create an empty matrix to include only data of genotype of individuals
        for line in population: # for each individual of the population
            w += 1  # assign a number to each line
            line = line.replace('   ', '').replace('\t', ' ').rstrip() # delete extra spaces
            if w != 1:
                if line.startswith('1') or line.startswith('2'): # if line starts with 1 0r 2 it indicates
                    i += 1  # assign a number to each individual
                    if i == 1:
                        pop = line
                    else:
    ...
    for g in range(Ngen):
        ...
        for y in range(1):
            ...
            subStructure = np.vstack([subStructure1, subStructure2])                                 # unify subsample of the 2 populations
            ...
            output = 'qvalues' + '_' + str(fst) + str(allele_number) + str(m) + str(N1) + str(N2) + str(Nmarker) + '_' + str(num_pop)
            ...
            output1 = 'subsampleStructure' + '_' + str(fst) + str(allele_number) + str(m) + str(N1) + str(N2) + str(Nmarker) + '_' + str(num_pop)
...
for filename in glob.glob(os.path.join(path, 'subsampleStructure*')):
    ...
    for x in range(50):
        if x == 0:
            ...
            sum1 += float(vectors[x, 0])  # sum of values in column 1
            sum2 += float(vectors[x, 1])  # sum of values in column 2
    ...
    if sum1 > sum2:
        ...
        qvalues = matrix_Structure[:, 1]     # if sum of column 1 is bigger than column 2, get q values and CI from column 2 and 4
        qvalues = np.ndarray.tolist(qvalues)
        qvalues = [float(t) for t in qvalues]
        intervals = matrix_Structure[:, 4:6]

    replicates = [w] * (N1 + N2)
    individual_number_list = [N1 + N2] * (N1 + N2)
    real_q = None
...
output2 = 'Fst' + str(fst) + '_NAl' + str(allele_number) + '_NMar' + str(Nmarker) + '_M' + str(m) + '_N' + str(N1 + N2)
